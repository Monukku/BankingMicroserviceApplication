...
class AccountServiceTest {
    ...
    @Test
    void createAccount_ShouldCreateAccountSuccessfully() throws AccountException, JsonProcessingException {
        // Arrange
        AccountService spyService = spy(accountService);
        doReturn("123456789012").when(spyService).generateAccountNumber();
        
        when(accountsRepository.findByCustomerIdAndDeletedAtIsNull(customerId))
                .thenReturn(List.of());
        doReturn("{}").when(objectMapper).writeValueAsString(any());
        
        // Mock the save operation to set ID on the account (simulating JPA behavior)
        doAnswer(invocation -> {
            Account accountToSave = invocation.getArgument(0);
            accountToSave.setId(UUID.randomUUID()); // Set ID like JPA would
            return accountToSave;
        }).when(accountsRepository).save(any(Account.class));

        // Act
        AccountResponse response = spyService.createAccount(keycloakUserId, customerId, createRequest);
